skill_tutien_truytamtien = class({})
require('kem_lib/kem')
SETTING_EFFECT = "particles/edited_particle/tu_tien/ttt.vpcf"
SETTING_FLY_TIME = 1 
...
--------------------------------------------------------------------------------
function skill_tutien_truytamtien:GetManaCost()
  local caster = self:GetCaster()
  local skill_level = self:GetLevel()+GetSkillLevel(caster)
   return skill_level*2
end

function skill_tutien_truytamtien:GetCooldown()
   local atk_perseconds = self:GetCaster():GetAttacksPerSecond()
   local cd = 1 / atk_perseconds
   return cd
end

function skill_tutien_truytamtien:OnAbilityPhaseStart()
   local atk_perseconds = self:GetCaster():GetAttacksPerSecond()
   self:GetCaster():StartGestureWithPlaybackRate(ACT_DOTA_ATTACK,atk_perseconds)
   return true
end

function skill_tutien_truytamtien:OnSpellStart()
   local caster = self:GetCaster()
   local skill_level = self:GetLevel() + GetSkillLevel(caster)
...
local max_target = 3
   ...
   DamageHandler:MissileHandler({attacker=proj.Source,target=unit,projectile=proj,hit_function=function(proj,unit)
      ...
      print("call apply poison "..proj.poison)
